...
def spectral_distill_over_L_cos_3bands(
    ...
    *,
    ...
    if recorder is not None and layer_idx is not None:
        recorder.update(layer_idx, student, teacher,
                        eps=eps, zero_mean=zero_mean, scale_group=8)    
    return loss_cos
...
recorder = SpectralCosStatsRecorder()
    ...
    def forward(
        ...
        **kwargs,
        ...
        if attention_mask is None:
            ...
            dis_loss = spectral_distill_over_L_cos_3bands(path_attn_scores.unsqueeze(1), spectral_teacher_scores.unsqueeze(1), self.layer_idx)
            if self.layer_idx == 11:
                layer_band_stats = recorder.get_layer_mean()
                plot_spectral_cos_heatmaps(layer_band_stats, out_dir="spectral_plots")
