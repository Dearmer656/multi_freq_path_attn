...
def parallel_path_fwd_kernel(
    ...
    decay_table,                  # 衰减表：建议 [HQ, T]（每个 head×rank 一行，长度=全序列）
    USE_DECAY: tl.constexpr,      # 编译期开关：是否启用衰减    
):
    ...
    for offset in range((i_t + 1) * BT - 2 * BS, i_t*BT-BS, -BS):
        ...
        if USE_DECAY:
            qi = (i_t * BT + tl.arange(0, BT))[:, None]
            kj = (offset + tl.arange(0, BS))[None, :]
            delta = (qi - kj).to(tl.int32)
            delta = tl.maximum(delta, 0)
            row_ptr = decay_table + i_hq * T   # decay_table 形状: [HQ, T]
            kappa = tl.load(row_ptr + delta, boundary_check=(0,))  # [BT,BS]        
        # [BT, BS]
        m_s = i_t * BT + tl.arange(0, BT) >= (offset + BS)
        b_s = tl.dot(b_q.to(b_k.dtype), b_k)
        if USE_DECAY: b_s = b_s * kappa
        if USE_GATE:
