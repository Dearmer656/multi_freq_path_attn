...
def chunk_transform_qk_bwd_kernel_prepare(
    q, k, v, w, beta, g_cumsum, L, D, h, q_new, k_new, AT, dA_local, dv, do, dg_cumsum, scale,
    indices, offsets, chunk_offsets,
    T,
    G: tl.constexpr, HQ: tl.constexpr, H: tl.constexpr,
    K: tl.constexpr, V: tl.constexpr,
    BK: tl.constexpr, BV: tl.constexpr, BT: tl.constexpr,
    IS_VARLEN: tl.constexpr, USE_GATE: tl.constexpr,
    RETURN_H: tl.constexpr,
    # ==== 新增：衰减表（块内 tail）与开关 ====
    decay_table,                 # [H*R, BT] 或 [HQ, BT]
    ...
    # wbk 基项（不含系数）
    b_wbk_base = tl.dot(b_w.to(b_kt.dtype), b_kt).to(b_q.dtype)  # [BT,BT]

    # 组装 coeff
    if USE_DECAY:
        # Δ = i - s，clamp 到 [0, T_tile-1]
        delta = (o_i[:, None] - o_i[None, :]).to(tl.int32)
        delta = tl.maximum(delta, 0)
        delta = tl.minimum(delta, T_tile - 1)
        # 选择当前 head×rank 的行（若 rank 合并到 HQ，则可直接用 i_hq）
        i_hr = i_h * G + (i_hq % G)
        ...
        coeff = b_beta[None, :].to(b_q.dtype)                      # 仅按列 β（退化为原逻辑）
    ...
    # q ← q − ((qwT ⊙ coeff) @ w)
    weighted = (b_qwT * coeff).to(b_w.dtype)
    b_q = b_q.to(tl.float32) - tl.dot(weighted, b_w.to(weighted.dtype))
    ...
    # 首个 query-rank 写 w2、k_new（保留仅 β 的结构通道）
    if i_hq % G == 0:
        b_Twb = tl.dot(b_T, b_w)  # 仅 β
    ...
    # softmax 的局部量（为 inter bwd 准备 dA_local 与 dv）
...
def intra_chunk_preprocess_bwd_prepare_fn(
    q, k, v, w, beta, g_cumsum, A, L, D, do, scale,
    return_h=True, cu_seqlens=None,
    decay_tail=None   # ← 新增，形状 [H*R, BT] 或 [HQ, BT]
):
    BT = A.shape[-1]
    HQ = q.shape[-2]
    B, T, H, K = k.shape
    ...
    USE_DECAY = decay_tail is not None

    chunk_transform_qk_bwd_kernel_prepare[grid](
        q=q, k=k, v=v, w=w, beta=beta, g_cumsum=g_cumsum,
        AT=A, dA_local=dA_local, dv=dv, dg_cumsum=dg_cumsum,
        do=do, L=L, D=D, h=h, q_new=q_new, k_new=k_new, scale=scale,
        offsets=cu_seqlens, indices=indices, chunk_offsets=chunk_offsets,
        T=T, H=H, G=G, HQ=HQ, K=K, V=V,
        BK=triton.next_power_of_2(K), BV=triton.next_power_of_2(V), BT=BT,
        IS_VARLEN=cu_seqlens is not None, USE_GATE=g_cumsum is not None,
        RETURN_H=return_h,
        decay_table=decay_tail if USE_DECAY else w,  # 占位也要传个指针；不用时不会解引用
        USE_DECAY=USE_DECAY
    )
